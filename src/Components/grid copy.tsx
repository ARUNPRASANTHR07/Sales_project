...
export interface GridColumn {
  ...
  width?: string;
  editable?: boolean;
  options?: { label: string; value: any }[]; // for combo
  ismandatory?: boolean;
}
...
const Grid: React.FC<GridProps> = ({
  ...
  const renderCell = (row: GridRow, column: GridColumn) => {
    ...
    switch (column.type) {
      ...
      case "combo":
        // 1. Convert string options to Array if they arrived as a string
        ...
        try {
          if (column.options) {
            if (Array.isArray(column.options)) {
              ...
              // If it's a string (like your JSON shows), parse it
              comboOptions = JSON.parse(column.options);
            }
          }
        } catch (error) {
          console.error("Error parsing combo options for column:", column.key, error);
          comboOptions = []; // Fallback to empty array to prevent crash
        }
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="flex gap-4 sticky top-0 bg-white dark:bg-gray-900 py-2 z-20">
        ...
        <div className="flex items-center" title="Export To Excel">
          <Export columns={columns} rows={gridData} fileName={filename}>
